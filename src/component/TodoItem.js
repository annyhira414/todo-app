  ...
  return (
    ...
    >
      <Box>
        ...
        <Box mt={1}> Deadline: {`${todo?.date?.startDate} - ${todo?.date?.endDate}`}</Box>
        <Box mt={1}>
          <b
            style={{
              color: todo?.status === "new" ? "blue" : todo?.status === "ongoing" ? "#f8a604" : "#44981a",
            }}
          >
            {todo?.status === "new" ? "New" : todo?.status === "ongoing" ? "Ongoing" : "Done"}
          </b>
          {todo?.status === "ongoing" && (
            <Box>
      ...
      <Box>
        ...
        {todo?.status === "new" ? (
          <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
            <MenuItem onClick={() => moveTodo(todo?.id, "ongoing")}>Ongoing</MenuItem>
